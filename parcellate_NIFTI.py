...
def parcellate_nifti(files_to_load):
    ...
    atlas_cort = load_data(osp.join(ATLAS_DIR, 'HarvardOxford-cort-resampled.nii.gz'), keepdims=True)
    atlas_subcort = load_data(osp.join(ATLAS_DIR, 'HarvardOxford-subcort-resampled.nii.gz'), keepdims=True)
...
def create_parcellated_data(atlas_cort, atlas_subcort, cort_labels, files_to_load, n_cort_labels, n_files,
    ...
    for i, file_path in enumerate(files_to_load):
        ...
        subj_data = load_data(file_path, keepdims=True)
